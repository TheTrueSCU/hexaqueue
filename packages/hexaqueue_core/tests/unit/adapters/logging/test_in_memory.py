...
async def test_in_memory_log_stream():
    ...
    await log_port.write_log(LogChunk(job_id="job-1", stream="stdout", content="line 1\n", offset=0))
    await log_port.write_log(LogChunk(job_id="job-1", stream="stdout", content="line 2\n", offset=1))
